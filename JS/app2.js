...
function gameMenu() {
    ...
    // Setup play/pause toggle on buttonOne
    buttonOne.innerText = "PAUSE"; // default to pause since music is playing
    buttonOne.style.display = "inline-block"; // ensure visible
    
    buttonOne.removeEventListener("click", audioPause); // remove previous if any
...
function theAnnouncement() {
    ...
    buttonTwo.style.display = "inline-block"; // ensure visible

    buttonTwo.removeEventListener("click", audioPauseTwo);
    ...
    nextBtn.removeEventListener("click", announcerSpeech);
    ...
    nextBtnTwo.removeEventListener("click", crazpiccSpeech);
    ...
    noGameStart.removeEventListener("click", battleOver);
    ...
    function theEnd() {
        ...
        cowardBtn.removeEventListener("click", goodbye);
        cowardBtn.addEventListener("click", goodbye);
        function goodbye() {
            window.close();
        }

        comebackBtn.removeEventListener("click", startAnew);
...
// Adjust elements scale on smaller screens dynamically
function handleResize() {
    const width = window.innerWidth;
    const mainEnemy = document.getElementById("mainEnemy");
    const announcer = document.getElementById("announcerGuy");

    if (!mainEnemy || !announcer) return;

    if (width < 600) {
        mainEnemy.style.transform = "scale(0.6)";
        announcer.style.transform = "scale(0.7)";
        // You can adjust font sizes or other style properties here too
    } else if (width < 900) {
        mainEnemy.style.transform = "scale(0.8)";
        announcer.style.transform = "scale(0.85)";
    } else {
        mainEnemy.style.transform = "scale(1)";
